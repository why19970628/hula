...
class ArticleSerializer(serializers.ModelSerializer):
    ...
    tag_name = serializers.CharField(source='tag.name', required=False)
    ...
    # ststus 2.变为中文
    status_txt = serializers.CharField(
        source='get_status_display', required=False)

    class Meta:
        model = models.Article
        fields = ['id', 'title', 'summary', 'content', 'category',
                  'category_name', 'x1', 'status', 'status_txt', 'x2', 'tag_name']  # category category_name可覆盖
        # fields = "__all__"
